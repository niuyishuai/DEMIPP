function [y,fval,iter,time] = proc_houboltscheme(J,x,y0,param)
    ...
    % start plots
    [Feps,fval1] = startplot(J,x,u0,u1,N,c,epsilon,plotflag,surfflag);
    ...
    while(Deltaf>tolf && Deltau > tolu)
        ...
        if explicitflag==false
            % using fsolve
            A=1/epsilon;
            B=(2*m/tau^2) +  (3*gamma/(2*tau)) - A;
            P=2*Un - Unm1;
            C=(m/tau^2)*(- 5*Un + 4*Unm1 - Unm2) + (gamma/(2*tau))*(-4*Un + Unm1) + c*P;
            gradval=evalfcn(grad,x,P);
            F=@(xx)A*xx.^3 + B*xx + C + gradval; 
            % Please be very very careful to avoid the recomputation of grad in F, 
            % which yields inefficiency in the next formulation of F.
            %F=@(xx)(m/tau^2)*(2*xx - 5*Un + 4*Unm1 - Unm2) + (gamma/(2*tau))*(3*xx-4*Un + Unm1) + ...
            %    (1/epsilon)*(xx.^3 - xx) + c*(2*Un - Unm1) + evalfcn(grad,x,2*Un-Unm1);
            [Unp1]=fsolve(F,Un,options); % solve nonlinear system for Un+1
        else
            % using Cardano's formula
            C=(m/tau^2)*(- 5*Un + 4*Unm1 - Unm2) + (gamma/(2*tau))*(-4*Un + Unm1) + c*(2*Un - Unm1);
            P=2*Un-Unm1;
            q=epsilon*(C + evalfcn(grad,x,P));
            qo2=-q/2;
            D=sqrt(qo2.^2 + (p/3)^3);
        ...
        % continue plots
        [fvalb1]=continueplot(Feps,x,Un,Unp1,fval,fval1,N,plotflag,surfflag);
    ...
    y = Unp1(:);
    fval=evalfcn(J,x,round(y)); % compute objective value at rounding point
